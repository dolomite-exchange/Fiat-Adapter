...
| blockedUsStates      | If `US` is an allowed region, a list of `US` states and territories (two letter state code) that are blocked. All others will be allowed. | []  what happens if I specify both allowed states and unallowed? Note: suggested re-name. |
